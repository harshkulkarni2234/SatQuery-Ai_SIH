...
| **Single image** — "Show me the built-up area." | Load card → Analyze → View Analysis | Grounding specialist (deterministic CV) draws **8 bounding boxes** over the urban areas, confidence 0.472 (bbox fill ratio — a real measured quantity, not a learned-model guess). **Do not ask this scene for water.** Its river is grey-brown rather than blue, so the HSV detector honestly reports "No water regions detected" — verified 2026-09-21. That refusal is correct behaviour and a fair talking point, but it is a weak opening demo. Also avoid farmland and vegetation on this image: both are so widespread that they collapse to a single box covering the whole frame, which shows nothing. Built-up is the only target here that produces real, localised regions. |

...
These wrap vqa.py / grounding.py / cross_modal.py (change detection is
wrapped in change_specialists.py) without editing them. Once those services return SpecialistResult natively
...
from app.services import change_specialists, cross_modal, grounding, vqa

# Change-detection handlers live in services/change_specialists.py. They are
# re-exported here so the registry keeps a single adapter namespace.
run_deterministic_change = change_specialists.run_deterministic_change
run_learned_change = change_specialists.run_learned_change
